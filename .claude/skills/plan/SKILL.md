...
- Affected resources and layers (entity, DTO, repository, service, controller, frontend)
- Acceptance criteria
...
- The closest analogue to what you're building (e.g., building CSV export → read the
  `com.schedulr.export` package and `MeetingController`).
- Load relevant `.claude/context/` modules (see the on-demand table in `CLAUDE.md`).

Identify:
- Files to modify (with line refs)
- New files to create (DTO records, entity, repository, service, controller, exception classes)
- Flyway migration needed? (any new table or column → yes, new versioned file, never edit a shipped one)
...
- Auth path (new endpoint → must go through the JWT `SecurityFilterChain`, no legacy bypass)
- Test coverage gap (what's not tested that could regress)

### 4. Write the plan to a file

Use the **Write tool** to create the file `plans/<feature-slug>-plan.md` — this is a
required deliverable, not optional. Do **NOT** just print the plan in your response;
`/implement` reads it from disk, so the file must exist. Use this structure:
