...
Delegates the implementation diff to the `code-reviewer` sub-agent, which checks it against `CLAUDE.md` rules (naming, JPA/DI patterns, CSV escaping, JWT auth, migration hygiene). Writes the verdict to `reports/<feature>-review.md`.
...
- A reminder to check constructor injection, `@Transactional` placement, CSV escaping, and JWT-only auth
